...
trait.desc.stats <- function(var, assaymethod) {
  ...
  datatable(proc_means(m.df, vars = as.character(var.name), by = as.character(assay.name), n=TRUE, nmiss = TRUE, mean=TRUE,
...
input_file <- "J:/endocrinology/Dunaif_Team/Dunaif_Lab/Subtyping/Clustering/GitHub/SampleData_355.txt"
...
clus.BMI.avg <- m.df_km %>%
  group_by(cluster) %>%
  summarise(BMI = mean(BMI))
met_km <- which.max(clus.BMI.avg$BMI)
clus.LH.avg <- m.df_km %>%
  group_by(cluster) %>%
  summarise(LH = mean(LH))
rep_km <- which.max(clus.LH.avg$LH)
...
#write.csv(clusters_km,"J:/endocrinology/Dunaif_Team/Dunaif_Lab/Subtyping/Clustering/GitHub/Kmeans_cluster_groups.csv")
...
##Label clusters by name
clus.BMI.avg <- m.df_gmm %>%
  group_by(cluster) %>%
  summarise(BMI = mean(BMI))
met_gmm <- which.max(clus.BMI.avg$BMI)
clus.LH.avg <- m.df_gmm %>%
  group_by(cluster) %>%
  summarise(LH = mean(LH))
rep_gmm <- which.max(clus.LH.avg$LH)
...
## Merge the sample_ids
hc.melt_gmm <- melt(clusters_km[1:9], id.var = "cluster_id")
...
CrossTable(clusters_consensus$consensus, clusters_consensus$clusters_hc, digits=2, prop.r = TRUE, prop.c =TRUE,
           prop.t= FALSE, prop.chisq = FALSE, chisq =FALSE, fisher =FALSE, missing.include=FALSE)

CrossTable(clusters_consensus$consensus, clusters_consensus$clusters_km, digits=2, prop.r = TRUE, prop.c =TRUE,
           prop.t= FALSE, prop.chisq = FALSE, chisq =FALSE, fisher =FALSE, missing.include=FALSE)

CrossTable(clusters_consensus$consensus, clusters_consensus$clusters_gmm, digits=2, prop.r = TRUE, prop.c =TRUE,
           prop.t= FALSE, prop.chisq = FALSE, chisq =FALSE, fisher =FALSE, missing.include=FALSE)
